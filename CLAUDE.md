...
- `lua/mappings.lua` — Global keymaps (`\` for NvimTree, `<leader>q` for diagnostics). Herdr-aware Neovim window navigation, including NvimTree, uses `<C-h/j/k/l>` via `herdr-nvim-nav`.
...
- **Window navigation** uses `herdr-nvim-nav` for seamless `<C-h/j/k/l>` movement across Neovim windows and Herdr panes; tmux fallback is disabled.
